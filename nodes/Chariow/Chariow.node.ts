		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'affiliate') {
					if (operation === 'get') {
						responseData = await affiliate.get.execute.call(this, i);
					} else if (operation === 'sendInvitations') {
						responseData = await affiliate.sendInvitations.execute.call(this, i);
					}
				}
				...
				if (resource === 'pulse') {
					if (operation === 'get') {
						responseData = await pulse.get.execute.call(this, i);
					} else if (operation === 'getAll') {
						responseData = await pulse.getAll.execute.call(this, i);
					}
				}
